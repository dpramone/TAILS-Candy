...
install()
{
	...
	wait
	if [ $? -ne 0 ]; then
		wget -O wizard.tar.gz.asc http://onionmail.info/network/wizard.tar.gz.asc || error_exit "Unable to download OnionMail signature file. Bailing out ..."
		wait
...
clear
echo "This routine will set up a persistent Onionmail account."
...
echo "3) You can run this script as many times as you want."
echo
...
if [ ! -f "$clawsmail" ]
	echo "Temporarily installing Claws Mail to make things work ..."
	sudo apt-get install claws-mail claws-mail-pgpinline claws-mail-pgpmime
fi
...
echo
echo "All set. Now set up a new IceDove mail account with the settings from"
echo "~/Persistent/Onionmail/onionmail-info-account*.txt ."
echo 
